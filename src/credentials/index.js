...
let SubMenu = () => {
    ...
    let menuCred = {
                ...
                update: [
...

                    {
                        ...
                        btn: async ({ chat_id }) => {
                            let user = infoUser().find(item => item.chat_id == chat_id)
                            let list = infoData().find(item => item.id == user.currentDataId)
                            let data = await financialDbController.getCurrentRate('UZS', get(list, 'startDate', ''))
                            let rate = data[0]?.Rate
                            if (rate) {
                                updateData(user.currentDataId, { currencyRate: rate })
                            }
        ...
        3: [
                ...
                update: [
...

                    {
                        ...
                        btn: async ({ chat_id }) => {
                            let user = infoUser().find(item => item.chat_id == chat_id)
                            let list = infoData().find(item => item.id == user.currentDataId)
                            let data = await financialDbController.getCurrentRate(get(list, 'currency', 'UZS'), get(list, 'startDate', ''))
                            let rate = data[0]?.Rate
                            if (rate) {
                                updateData(user.currentDataId, { currencyRate: rate })
                            }
            ...
            },
            {
                ...
                update: [
                    ...
                    //             let b1Account15 = await financialDbController.getAccount15()
        ...
        4: [
            ...
            {
                name: "BX Shartnoma shabloni",
                comment: "Sana:\nOfisdan dogovor so'rash shabloni.\n-Firma: Bolter (Unikus)\n-Mijoz: Ismi\n-Buxgalter ismi va telefon raqami: Tel nomeri\n-INN: \n-Shartnoma turi: ochiq\n-Shartnoma summasi: ?? mln\n-Ishonchnoma summasi : ?? mln\n-Ishonchnoma nomeri va sanasi: (01.01.2025)\n-To'lov summasi:\n-Tovar narxi: (Bizni prixodga qarab)\n-Tovar nomi:\n\nIzoh: Bo'lgan ish sababini to'liq bayon qilib yozing!\n\n#shartnomaBX\n\n@XusravRasulov\n@TolanovTolqin",
                update: [
                    {
                        id: 1,
                        name: "Izoh",
...
let excelFnPaymentLines = async ({ main }) => {
    ...
    let cashflow = await financialDbController.cashFlowList(list)
...
async function generateUsersPermissionsExcel(users, permissions, menus, filePath) {
    ...
    sheet.columns = [
        { header: 'Ism Familiya', key: 'fullName', width: 35 },
        { header: 'Rollar', key: 'roles', width: 30 },
        { header: 'Xodim', key: 'emp', width: 40 },
        { header: 'Tasdiqlovchi', key: 'affirmative', width: 40 },
        { header: 'Bajaruvchi', key: 'executor', width: 40 },
    ];

    for (const user of users) {
        const perm = permissions.find(p => p.chat_id == user.chat_id);
        if (!perm) continue;

        for (const [menuId, subMenus] of Object.entries(menus)) {
            subMenus.forEach((sm, idx) => {
                sheet.addRow({
                    fullName: `${user.FirstName} ${user.LastName}`,
                    roles: mapRoles(perm.roles || []),
                    emp: perm.permissonMenuEmp?.[menuId]?.includes(String(idx + 1)) ? sm.name : '-',
                    affirmative: perm.permissonMenuAffirmative?.[menuId]?.includes(String(idx + 1)) ? sm.name : '-',
                    executor: perm.permissonMenuExecutor?.[menuId]?.includes(String(idx + 1)) ? sm.name : '-',
                });
            });
        }
    }
